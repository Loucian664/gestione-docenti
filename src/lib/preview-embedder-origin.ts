...
/** Live Grok preview iframe — storage here is not the published site. */
export function isUnstablePreviewHost(): boolean {
  ...
  try {
    ...
    if (window.parent !== window) {
      ...
      if (ref) {
        ...
        if (isGrokEmbedderOrigin(origin)) return true;
      }
    }
  } catch {
    // ignore
